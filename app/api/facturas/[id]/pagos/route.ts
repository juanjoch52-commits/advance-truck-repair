...
import { INVOICE_COLS, RECEIPT_METHODS, round2, deriveBalanceStatus } from '@/lib/invoicesApi';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: invoice, error: invErr } = await supabase.from('invoices').select('id,total,amount_paid,status').eq('id', id).maybeSingle();

  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="display-font text-3xl font-bold text-slate-100 tracking-wide">MECANICOS</h1>
          <p className="text-slate-400 mt-1">{employees.length} empleados registrados</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-slate-900/80 border border-amber-500/20 rounded-xl p-6 mb-6">
          <h2 className="display-font text-slate-200 font-semibold mb-4 tracking-wide">REGISTRAR MECANICO</h2>
          <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="mecanico@taller.com"
                className="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-2.5 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-amber-400/50 transition" />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <input value={notes} onChange={e => setNotes(e.target.value)} placeholder="Especialidad, observaciones..."
                className="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-2.5 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-amber-400/50 transition" />
            </div>
            {error && (
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/60 border border-white/5 rounded-xl p-12 text-center">
          <p className="text-slate-500">No hay mecanicos registrados.</p>
        </div>
      ) : (

...
const TOURS: Record<string, Step[]> = {
  ...
  'facturacion-nueva': [
    ...
    S('[data-tour="facn-client"]', 'Cliente', 'Customer',
      '“Registrado” = un cliente que ya está en tu lista (aparecen su distrito y camión si los tiene). Si no está, usa “Registrar cliente nuevo” para crearlo aquí mismo. “Ocasional” = alguien de una sola vez, cuyo nombre escribes a mano. Si el cliente es exento, el impuesto se pone en $0 solo.',
      '“Registered” = a customer already in your list (their district and truck show if they have them). If they’re not there, use “Register new customer” to add them right here. “Walk-in” = a one-time customer whose name you type. If the customer is exempt, tax is set to $0 automatically.'),
    ...
    S('[data-tour="facn-labor"]', 'Mano de obra', 'Labor',
      'Cada trabajo: qué se hizo, la cantidad y el precio por unidad (ej. cambio de llanta × 4). El total del renglón se calcula solo. La mano de obra normalmente no lleva impuesto en Florida (la casilla está apagada). Abajo eliges quién trabajó (hasta 2 mecánicos) y su % de comisión: de ahí sale su pago a la planilla.',
      'Each job: what was done, the quantity and the price per unit (e.g. tire change × 4). The line total is computed for you. Labor usually isn’t taxed in Florida (the box is off). Below, pick who worked (up to 2 mechanics) and their commission %: that’s where their payroll pay comes from.'),
